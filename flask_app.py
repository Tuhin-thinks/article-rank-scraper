...
def upload():
    ...
    if request.method == "POST":
        ...
        if extension in (".csv", ".xlsx"):
            flash("File uploaded!")
            # create time hash as file name and save to uploads folder
            time_hash = hashlib.md5(datetime.now().isoformat().encode("utf-8")).hexdigest()
            __file_name = f"{time_hash}.{extension}"
            __file = os.path.realpath(os.path.join("uploads", __file_name))
            f.save(__file)
            res = check_contains_columns(__file)
            if not res:
                flash("File must have a column named 'Title' containing all "
